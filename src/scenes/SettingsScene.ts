...
export class SettingsScene extends Phaser.Scene {
  ...
  private renderOverlay(): void {
    ...
    const html = createSettingsSceneOverlayHtml({
      ...
      // v0.3.56: GP Achievements кнопка временно убрана — GP overlay API
      // не публичен, кнопка ни на что не реагирует. Достижения трекаются
      // в фоне через AchievementsReconciler + GP-toasts при unlock'ах.
