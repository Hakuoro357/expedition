...
```ts
// R6 codex (NaN-safety): защита от non-finite SDK/persisted values.
...
const sdkProgress = readProgress(sdkProgressByTag.get(meta.tag));
const persistedProg = readProgress(persistedProgress?.[meta.tag]);
