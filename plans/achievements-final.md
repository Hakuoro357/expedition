...
4. **Исследовать GP Schedulers** — https://docs.gamepush.com/ru/docs/schedulers/
   (планировщики наград: что это, как использовать для retention, можно ли
   привязать к ачивкам / daily-claim / login-streak).
5. **Исследовать GP Reviews** — https://docs.gamepush.com/ru/docs/application/reviews/
   (отзывы игроков: API запроса review-prompt после ключевых событий —
   например, после `epilogue` unlock или `chapter_3_complete`).
