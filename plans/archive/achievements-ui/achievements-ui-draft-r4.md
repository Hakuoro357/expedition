# Achievements UI page + entry points (v0.3.58) — draft-r4
...
async function AchievementsScene.create() {
  ...
  // 1. Сразу рендерим из compute() + persistedUnlocked (save).
...
**R2+R3: unlocked + compute<max → clamp display** — если unlocked (любой источник), показываем
`progress = max` вместо реального compute(), чтобы избежать «✓ 3/10» broken-look.
**R3 fix codex-M4**: формула применяется **только** для max-ачивок. One-shot (без max)
не имеют progress-bar совсем — просто ✓ если unlocked, locked-силуэт если нет:

```ts
if (typeof meta.max === "number") {
  const rawCompute = typeof compute === "number" ? compute : 0;
  const displayProgress = isUnlocked ? meta.max : Math.min(rawCompute, meta.max);
  vm.displayProgress = displayProgress;
  vm.displayPct = (displayProgress / meta.max) * 100;
  // VM.progressbar отрендерится в overlay
} else {
  // one-shot — progressbar отсутствует
...
- One-shot achievements (no max): unlocked iff compute()===true, **NO progressbar in VM** (R3 codex-M4)
...
.route-overlay__community {
  position: absolute;
  top: calc(env(safe-area-inset-top, 0px) + var(--top-pad) + var(--top-action-size) + var(--top-action-gap));
  right: calc(env(safe-area-inset-right, 0px) + var(--top-pad));
  width: var(--top-action-size); height: var(--top-action-size);
  z-index: 100;
}
.route-overlay__achievements {
  position: absolute;
  /* если community нет — trophy сразу под mute. Управляется через CSS-cls
     `--has-community` на overlay-root, переключающим top offset. */
  top: calc(env(safe-area-inset-top, 0px) + var(--top-pad) + (var(--top-action-size) + var(--top-action-gap)) * 2);
  right: calc(env(safe-area-inset-right, 0px) + var(--top-pad));
  width: var(--top-action-size); height: var(--top-action-size);
  z-index: 100;
}
