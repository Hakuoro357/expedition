# Achievements UI page + entry points (v0.3.58) — draft-r3
...
async function AchievementsScene.create() {
  ...
  try {
    ...
    // SDK fail → остаёмся с compute-only view. Лог в console.warn.
...
**R2: unlocked + compute<max → clamp display** — если unlocked (любой источник), показываем
`progress = max` вместо реального compute(), чтобы избежать «✓ 3/10» броken-look:

```ts
const displayProgress = isUnlocked ? meta.max : Math.min(compute, meta.max);
const displayPct = meta.max ? (displayProgress / meta.max) * 100 : (isUnlocked ? 100 : 0);
```
...
для всех hidden ачивок. Создаём один раз, кладём в `public/assets/achievements/`.
